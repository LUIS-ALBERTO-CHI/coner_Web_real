...
class CatalogueService:
    ...
    def single(self, objWithId):
        kind = type(objWithId).__name__
        db = self.get_db_client()
        factory = Factory()
        newObj = factory.create(type(objWithId))
        
        if objWithId.id:
            ref = db.collection(kind).document(objWithId.id)
            data = ref.get()
            print(data.to_dict())
            newObj.__init__(**data.to_dict())
    ...
    def list(self, clazz = Model, filters = {}, cursor = None):
        kind = clazz.__name__
        factory = Factory()
        db = self.get_db_client()
        listWCursor = {
